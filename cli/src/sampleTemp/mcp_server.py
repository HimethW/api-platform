...
runner = ArazzoRunner.from_arazzo_path("./arazzo/multi_workflow_indep.yaml")
...
@mcp.tool()
async def update_pet_info(petId: int, newName: str) -> str:
    """Check that a pet exists, then update its name."""
    try:
        result = runner.execute_workflow("updatePetInfo", {"petId": petId, "newName": newName})
        if result.outputs:
            return f"Workflow Success. Outputs: {result.outputs}"
        return f"Workflow Result: {result}"
    except Exception as e:
